...
router.get('/creartabla', function(req, res, next) {
  bd.query('drop table if exists articulos',function
 (error,resultado){
  if (error) {
  console.log(error);
  return;
...
router.post('/alta', function(req, res, next) {
 ...
 bd.query('insert into articulos set ?',registro,
function (error,resultado){
 if (error){
 console.log(error);
 return;
...
bd.query('UPDATE articulos SET ? WHERE ?',[registro,{codigo:req.body.codigo}],
